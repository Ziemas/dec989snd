...
UInt32 snd_PlaySoundVolPanPMPB(SoundBankPtr bank, SInt32 sound, SInt32 vol, SInt32 pan, SInt16 pitch_mod, SInt16 bend) {
    ...
    if (bank->DataID == DATAID_SBLK) {
        return snd_PlaySFX((SFXBlock2Ptr)bank, sound, vol, pan, pitch_mod, bend);
    } else {
...
SInt32 snd_GetSoundOriginalPitch(SoundBankPtr bank, SInt32 sound) {
    ...
    if (!bank || bank->DataID == DATAID_SBLK || sound >= bank->NumSounds || sound < 0) {
        return 0;
    }
...
SInt32 snd_FindSoundByName(SFXBlock2Ptr block, char *name, SFXBlock2Ptr *found_block) {
    ...
    count = 0;
    if (block && (block->DataID != DATAID_SBLK || (block->Flags & 0x100) == 0)) {
        return -1;
    }
    ...
    while (snd_names[index].Name[0]) {
        count++;
        if (snd_names[index].Name[0] == buffer[0] && snd_names[index].Name[1] == buffer[1] &&
            snd_names[index].Name[2] == buffer[2] && snd_names[index].Name[3] == buffer[3]) {
            if (found_block) {
                *found_block = block;
            }
